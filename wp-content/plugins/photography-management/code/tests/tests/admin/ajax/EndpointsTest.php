...
class AjaxEndpointsTest extends WP_Ajax_UnitTestCase {
  ...
  private function makeCall($hook, $data = array()) {
    ...
    $_POST[\hacklib_id("payload")] = $payload;
    try {
    ...
    if (\hacklib_cast_as_boolean(is_null($json))) {
  ...
  private function assertSingleErrorWithMessage($msg, $res, $notice = null) {
  ...
  private function assertImageOk($image, $id, $minithumb = null) {
    ...
    if (\hacklib_cast_as_boolean($minithumb)) {
      $this->assertSame(
        $image->mini_thumb,
  ...
  public function testCheckEmail() {
    ...
    $this->assertErrorsWithMessages(
      array("The property email is required"),
      $res
    );
  }
  public function testCheckEmail2() {
    $res = $this->makeCall(
      "phmm_check_email",
      array("some" => "bs", "email" => 42, "client_id" => array())
    );
    $this->assertErrorsWithMessages(
      array(
  ...
  public function testCheckEmail3() {
    $res = $this->makeCall(
      "phmm_check_email",
      array("email" => "", "client_id" => "42")
    );
    $this->assertErrorsWithMessages(
      array("Must be at least 1 characters long"),
      $res
    );
  }
  public function testCheckEmail4() {
    ...
    foreach ($invalidAddresses as $email) {
      $res = $this->makeCall(
        "phmm_check_email",
        array("email" => $email, "client_id" => "42")
      );
      $this->assertSingleErrorWithMessage(
        "Invalid email",
        $res,
  ...
  public function testCheckEmail6() {
    ...
    $res = $this->makeCall(
      "phmm_check_email",
      array("email" => $email, "client_id" => $client->ID)
    );
    $this->assertTrue($res->success);
    $this->assertTrue($res->data->data);
  }
  ...
  public function testFetchGalleryImages5() {
    ...
    $this->assertSame(
      ...
      (array) \hacklib_cast_as_array($res->data->data[0])
    );
  ...
  public function testLabelImages9() {
    ...
    $this->assertTrue($res->success);
    $set = Labels::get_set($client->ID, 42, "1111111111111");
    $this->assertSame(array(21, 18, 15), $set);
  }
}
